...
export default function ChatInterface({ messages, setMessages, setActiveMessageId, onSourceClick, isSidebarOpen, suggestions, onSendMessage, isGenerating, onStopGenerating }) {
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      {/* Selection Gate Overlay */}
      {(!messages || (messages.length === 1 && messages[0].id === 'welcome' && chats?.find(c => c.id === currentChatId)?.selectedFiles?.length === 0)) && (
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            <div className={`flex flex-col max-w-[80%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
              ...
              >
                <div className="whitespace-pre-wrap text-[15px] leading-relaxed w-full">
                  {msg.isTyping ? (
                      ...
                      {msg.role === 'assistant' && msg.steps && msg.steps.length > 0 && (
                        <div className="mb-3 text-sm w-full block">
                          <button 
                            ...
                            className="flex w-full items-center justify-between text-slate-500 hover:text-indigo-600 transition-colors font-medium bg-slate-50/50 px-3 py-2 rounded-lg border border-slate-100 mb-2 shadow-sm"
                          >
                          ...
                          {openThoughts[msg.id] && (
                            <div className="mt-2 p-3 bg-slate-50 rounded-lg border border-slate-100 text-slate-600 text-xs space-y-2 mb-3 shadow-inner">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {(() => {
                    ...
                    msg.citations.forEach(cite => {
                      if (!uniqueSources[cite.source]) {
                        uniqueSources[cite.source] = cite;
                      }
                    });
